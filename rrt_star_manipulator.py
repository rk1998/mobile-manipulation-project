...
class Tree_manip():
    ...
    def find_neighbors(self, q, dictionary, radius, G):
      costs = {}
      Xbest = None
      min_cost = 10000000
...
def RRT_star(start_config, Qgoal, env, arm, lim=0.5, step_size=3, num_iters=1000):
    ...
    while curr_iter < num_iters:
        ...
        Xbest, costs = graph.find_neighbors(end_eff_rand, dictionary, radius, G)
        ...
        if end_eff_steer is not None:
            ...
            for idx in costs:
                qn = dictionary[idx]
                if min_dist + graph.find_dist2(qn, end_eff_rand) < costs[idx]:
                    curr_iter += 1
                    cost[idx] = min_dist + graph.find_dist2(qn, end_eff_rand)
